...
router.post('/entrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    return banco.sql.query(`select * from tbUsuario where loginUsuario='${login}' and senhaUsuario='${senha}'`);
  }).then(consulta => {
...
router.post('/cadastrar', function (req, res, next) {
  ...
  banco.conectar().then(() => {
    ...
    codFuncionario = req.body.codFuncionario;
    if (login == undefined || senha == undefined || codFuncionario == undefined) {
	  // coloque a frase de erro que quiser aqui. Ela vai aparecer no formulário de cadastro
      throw new Error(`Dados de cadastro não chegaram completos: ${login} / ${senha} / ${codFuncionario}`);
    }
    return banco.sql.query(`select count(*) as contagem from tbUsuario where loginUsuario = '${login}'`);
  }).then(consulta => {
    ...
    res.status(500).send(erro);
	  ...
	  if (cadastro_valido) {		  
		...
		banco.sql.query(`insert into tbUsuario (loginUsuario, senhaUsuario, codFuncionario) values (${login}','${senha}','${codFuncionario}')`).then(function() {
